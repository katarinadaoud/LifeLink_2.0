...
interface MedicationGridProps { 
  ...
  onDelete?: (medicationId: number, medicationName: string) => void;
      ...
      {medications.length === 0 ? (
        ...
        <Row xs={1} sm={2} md={3} lg={4} className="g-4">
          ...
          {medications.map(medication => (
            <Col key={medication.medicationId}>
                ...
                <Card.Body className="d-flex flex-column"> {/* vertical layout inside the card*/}
                  ...
                  <div className="mt-auto d-flex justify-content-end gap-2">
                    ...
                    {userRole === 'Employee' && (
                      <>
                        {onEdit && (
                          <Button onClick={() => onEdit(medication.medicationId!)} className="btn btn-teal">Update</Button>
                        )}
                        {onDelete && (
                          <Button onClick={() => onDelete(medication.medicationId!, medication.medicationName)} className="btn btn-delete">Delete</Button>
                        )}
                      </>
                    )}
                  </div>               
